...
- `promptune shell-init` raw `--key` passthrough is now validated by a strict character allowlist (`[A-Za-z0-9^[]\\_-]`) instead of an incomplete denylist; a crafted key such as `<(reboot)` can no longer reach the unquoted zsh `bindkey` / fish `bind` line and execute when the widget is eval'd (bash was already single-quoted)
- History database and its directory are now created owner-only (`0o600` / `0o700`), which also covers the WAL/SHM sidecars; prompts (stored verbatim, potentially sensitive) are no longer group/other-readable, matching the existing `0o600` config-file hardening
- Context sanitizer keyword redaction (`token=`, `api_key=`, `secret:`, …) now bounds the redacted value to non-whitespace instead of the rest of the line, so on the single ` | `-joined context line one such substring no longer redacts every later signal (errors, frameworks, …); real secrets remain caught by the high-entropy pass
- Tech-stack detection no longer discards already-detected languages when `package.json` / `pyproject.toml` is not valid UTF-8 — `UnicodeDecodeError` is now handled alongside `JSONDecodeError` / `OSError` (mirrors the existing null-dependencies guard)
- Quality scoring (and the displayed `promptune score`) now matches vague-verb, vague-word, and filler penalties on whole words instead of substrings: "document"/"output"/"target" no longer count as the vague verbs do/put/get, "every"/"awesome" no longer as the vague words very/some, "adjust" no longer as the filler "just". Fixes systematic under-scoring of actionability — which also skewed tier routing — and over-counted specificity/conciseness penalties
- `engine.enhance()` auto-routing now logs AI-tier failures instead of swallowing them: tier-1 (local) unavailability at `debug` (a benign fallback to tier 2), a tier-2 failure at `warning` (it degrades to tier 0). A persistent auth/config error previously left the AI tier silently disengaged with no diagnostic
